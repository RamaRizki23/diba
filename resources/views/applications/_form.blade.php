...
<div><label for="language">Bahasa pemrograman</label><select id="language" name="language"><option value="">-- Pilih bahasa --</option>@foreach(['PHP','JavaScript','TypeScript','Python','Java','C#','Go','Ruby','Kotlin','Dart'] as $language)<option value="{{ $language }}" {{ old('language', $application->language ?? '') === $language ? 'selected' : '' }}>{{ $language }}</option>@endforeach</select></div>
<div><label for="framework">Framework</label><select id="framework" name="framework"><option value="">-- Pilih framework --</option>@foreach(['Laravel','CodeIgniter','Vue.js','React','Next.js','Angular','Django','Spring Boot','.NET','Flutter'] as $framework)<option value="{{ $framework }}" {{ old('framework', $application->framework ?? '') === $framework ? 'selected' : '' }}>{{ $framework }}</option>@endforeach</select></div>
<div><label for="database">Database</label><select id="database" name="database"><option value="">-- Pilih database --</option>@foreach(['MySQL','PostgreSQL','SQLite','MongoDB','MariaDB','Microsoft SQL Server','Oracle','Redis'] as $database)<option value="{{ $database }}" {{ old('database', $application->database ?? '') === $database ? 'selected' : '' }}>{{ $database }}</option>@endforeach</select></div>
